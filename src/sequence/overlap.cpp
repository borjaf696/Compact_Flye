								...
								int maxOverlaps) const
{
	...
	timeStart = std::chrono::system_clock::now();
	int cont = 0, cont2 = 0;
	std::pair<size_t,select_support_mcl<1>>
		kmers_per_read = _vertexIndex.get_select_structure(fastaRec.id);
	select_support_mcl<1> select_local = kmers_per_read.second;

	/*for (const auto& curKmerPos : IterKmers(fastaRec.sequence))
	{*/
    //std::cout << "ReadId: "<<fastaRec.id.getId()<<" "<<kmers_per_read.first<<std::endl;
    if (kmers_per_read.first != 0)
		for (size_t i = 1; i <= kmers_per_read.first; i++)
		{
			size_t place = (size_t) select_local(i);
			KmerPosition curKmerPos(
					Kmer(fastaRec.sequence,place,Parameters::get().kmerSize)
					,place);
			//bool revcomp = curKmerPos.kmer.standardForm();
			cont2++;
			//std::cout<<"Check!"<<std::endl;
			int pos = _vertexIndex.check(curKmerPos.kmer);
			...
			if (!_vertexIndex.kmerFreq(curKmerPos.kmer)) continue;
				/*printf("CurKmerPos %lld %d %lld %lld %lld\n",curKmerPos.kmer.getRepresentation()
						,curKmerPos.position, _vertexIndex.kmerFreq(curKmerPos.kmer), curLen, kmers_per_read.first);*/
			/*exit(1);*/
			//FastaRecord::Id prevSeqId = FastaRecord::ID_NONE;
			for (const auto& extReadPos : _vertexIndex.iterKmerPos(curKmerPos.kmer, pos))
			{
				//no trivial matches
				//std::cout << extReadPos.readId.getId()<<" "<<extReadPos.position<<std::endl;
		...
		for (const auto& curKmerPos : IterKmers(fastaRec.sequence))
		{
			...
			int pos = _vertexIndex.check(curKmerPos.kmer);
			if (pos == -1) continue;
				/*printf("CurKmerPos %lld %lld %lld %lld\n",curKmerPos.kmer.getRepresentation()
						,curKmerPos.position, _vertexIndex.kmerFreq(curKmerPos.kmer), curLen);*/
